...
- AC-2: A checkpoint written by the driver round-trips through the policy-lift architecture resolution with no manual flags, with the sidecar as the source of truth: the `.meta.json` carries a `network` object from which `src.benchmark.policy_lift.load_architecture` reconstructs the network — sidecar-driven resolution, not the shape-inference or chess-default fallbacks — and the checkpoint's state_dict loads onto that network. Falsified by a missing, malformed, or ignored sidecar (resolution falling through to inference/defaults), by `load_architecture` raising, or by the state_dict failing to load (`RuntimeError`). Intended test: `tests/unit/training/test_self_play_convergence.py` (sidecar round-trip case asserting the resolved architecture matches the sidecar's `network` payload).
- AC-3: A run of `python -m src.benchmark.policy_lift --domain chess --checkpoint <driver-produced .pt> --num-games 100 --output benchmarks/results/m5_policy_lift.json` runs to completion (exit code 0 or 1 — gate met or not met, both valid results — never 2) and the JSON artifact is committed at `benchmarks/results/m5_policy_lift.json` with run provenance: domain `chess`, `num_games >= 100`, confidence `0.95`, target lift `20`, seed, and checkpoint identity. Intended test: `tests/integration/benchmark/test_m5_lift_artifact.py` (schema and provenance assertions on the committed artifact).
