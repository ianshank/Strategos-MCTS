...
`docs/related-work.md`. The long-running jobs in this repository are the benchmark
sweep (`python -m src.benchmark`) — which today holds all results in memory and
loses everything on a crash — and self-play convergence training, which already
has checkpoint/resume and is out of scope for new persistence work.
...
- AC-1: `GraphBuilder.build_graph()` validates the state schema and the wired
  ...
  (or carrying unknown keys, unless explicitly allowed) with
  `StateValidationError` before the first node executes. Falsified by any of
...
- AC-3: Every node transition of a graph execution emits a structured trace
  ...
  configured, `load_trace(root, run_id)` returns the ordered event sequence from
  which `reconstruct_path` yields the executed node path. Falsified by a
...
- AC-4: `IntegratedFramework` accepts an injected checkpointer and
  ...
  `langgraph-checkpoint-sqlite` dependency with graceful fallback when absent),
  and threads a caller-supplied per-job `thread_id` through `process()` in place
  of the previous hardcoded constant; graph state remains JSON-serializable (the
  MCTS node summary replaces the live tree object in state). Falsified by a
  hardcoded thread id, by an import error when the sqlite extra is absent, or by
  non-serializable values written into checkpointed state. Intended test:
...
- AC-5: A benchmark sweep persists each scored result durably before advancing
  ...
  results and without re-scoring completed cells, while the final `results.json`
  keeps its existing shape. Falsified by data loss after SIGKILL, duplicate
  cells after resume, re-execution of completed cells, or a changed
  `results.json` schema. Intended tests:
...
- AC-6: `docs/MIGRATION_NOTES.md` documents every externally observable change
  ...
  (benchmark run store, trace files, sqlite checkpoints) — and states that
  legacy `results.json` output and training checkpoints are unaffected.
  Falsified by a shipped behavior or format change absent from the note.
  Intended check: doc review in the implementing PR.
