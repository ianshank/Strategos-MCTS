...
- No src/** changes.
- AC-5 and AC-11 both widen the coverage denominator. Both MEASURED 2026-08-04 rather than assumed: AC-11 alone moves the gate-scope figure 89.87% -> 89.85% (restoring 245 units: +161 statements, +84 branches). AC-5 (dropping the three --ignore flags and installing the api extra) takes it to 89.65% on a denominator of 42,318 units, up from 41,471. Combined that is -0.22pp against a 4.65pp margin over the 85% gate, so the earlier "must land in a single commit or CI red-lines at 85%" constraint was over-cautious and neither AC forces the other's timing.
...
- The unit-only coverage number the CI gate actually measures differs from docs/STATUS.md's full-suite headline (CHARTER.md INV-5) and was unpublished before this spec; AC-6 publishes it. MEASURED baseline, `.[dev,neural]`, `pytest tests/unit/` with the three --ignore flags: 8473 passed / 62 skipped / 0 failed, 89.87% pre-AC-11 and 89.85% post-AC-11, wall time 3m12s. Post-AC-5 under `.[dev,neural,api]` without the --ignore flags: 89.65%.
