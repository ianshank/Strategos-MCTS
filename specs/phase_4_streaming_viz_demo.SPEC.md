...
- AC-1: MCTS early termination is config-driven through the graph builder path, gated by an
...
- AC-2: Streaming, graph-visualization, and comparison logic live in coverage-bearing service modules
...
- AC-3: The single source of truth for early-termination thresholds remains `MCTSConfig` (no duplicated
  settings values).
- AC-4: The Gradio app (`app.py`) exposes comparison + streaming + tree visualization via those services; a new
  `[ui]` extra declares `gradio`; the module imports without the extra installed.
- AC-5: New behavior is opt-in via settings flags; existing `/query`, demo, and default config are unchanged.
