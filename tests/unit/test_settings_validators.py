...
class TestSettingsValidators:
    ...
    def test_anthropic_key_too_short_rejected(self):
        ...
        with pytest.raises(Exception, match="too short"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "ANTHROPIC_API_KEY": "short"}
            ):
                Settings()
    ...
    def test_braintrust_key_placeholder_rejected(self):
        ...
        with pytest.raises(Exception, match="placeholder"):
            with patch.dict(
                os.environ,
                {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "BRAINTRUST_API_KEY": "REPLACE_ME"},
            ):
                Settings()
    ...
    def test_braintrust_key_too_short_rejected(self):
        ...
        with pytest.raises(Exception, match="too short"):
            with patch.dict(
                os.environ,
                {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "BRAINTRUST_API_KEY": "short"},
            ):
                Settings()
    ...
    def test_pinecone_key_placeholder_rejected(self):
        ...
        with pytest.raises(Exception, match="placeholder"):
            with patch.dict(
                os.environ,
                {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "PINECONE_API_KEY": "your-api-key-here"},
            ):
                Settings()
    ...
    def test_pinecone_key_too_short_rejected(self):
        ...
        with pytest.raises(Exception, match="too short"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "PINECONE_API_KEY": "x"}
            ):
                Settings()
    ...
    def test_langsmith_key_placeholder_rejected(self):
        ...
        with pytest.raises(Exception, match="placeholder"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "LANGSMITH_API_KEY": ""}
            ):
                Settings()
    ...
    def test_langsmith_key_too_short_rejected(self):
        ...
        with pytest.raises(Exception, match="too short"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "LANGSMITH_API_KEY": "short"}
            ):
                Settings()
    ...
    def test_wandb_key_placeholder_rejected(self):
        ...
        with pytest.raises(Exception, match="placeholder"):
            with patch.dict(
                os.environ,
                {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "WANDB_API_KEY": "your-api-key-here"},
            ):
                Settings()
    ...
    def test_wandb_key_too_short_rejected(self):
        ...
        with pytest.raises(Exception, match="too short"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "WANDB_API_KEY": "ab"}
            ):
                Settings()
        ...
        with pytest.raises(Exception, match="http"):
            with patch.dict(
                os.environ,
                {
                    "OPENAI_API_KEY": "sk-" + "a" * 50,
                    "LLM_PROVIDER": "openai",
                    "LMSTUDIO_BASE_URL": "ftp://localhost:1234",
                },
            ):
                Settings()
    ...
    def test_s3_bucket_too_short(self):
        ...
        with pytest.raises(Exception, match="3-63"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "S3_BUCKET": "ab"}
            ):
                Settings()
    ...
    def test_s3_bucket_hyphen_start(self):
        ...
        with pytest.raises(Exception, match="hyphen"):
            with patch.dict(
                os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "S3_BUCKET": "-mybucket"}
            ):
                Settings()
...
class TestSettingsAPIKeyGetters:
    ...
    def test_get_braintrust_api_key_present(self):
        key = "bt-" + "x" * 50
        with patch.dict(
            os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "BRAINTRUST_API_KEY": key}
        ):
            from src.config.settings import Settings
    ...
    def test_get_pinecone_api_key_present(self):
        key = "pc-" + "x" * 50
        with patch.dict(
            os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "PINECONE_API_KEY": key}
        ):
            from src.config.settings import Settings
    ...
    def test_get_langsmith_api_key_present(self):
        key = "ls-" + "x" * 50
        with patch.dict(
            os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "LANGSMITH_API_KEY": key}
        ):
            from src.config.settings import Settings
    ...
    def test_get_wandb_api_key_present(self):
        key = "wb-" + "x" * 50
        with patch.dict(
            os.environ, {"OPENAI_API_KEY": "sk-" + "a" * 50, "LLM_PROVIDER": "openai", "WANDB_API_KEY": key}
        ):
            from src.config.settings import Settings
