...
class CheckpointMixin:
    ...
    def _save_checkpoint(self, iteration: int, metrics: dict, is_best: bool=False):
        ...
        if is_best:
            ...
            try:
                ...
                logger.info('Best model checkpoint saved', best_model_path=str(best_path), iteration=iteration, win_rate=self.best_win_rate)
            except Exception as e:
    ...
    def load_checkpoint(self, path: str):
        ...
        try:
            ...
            logger.info('Checkpoint loaded successfully', checkpoint_path=path, iteration=self.current_iteration, best_win_rate=round(self.best_win_rate, 4), checkpoint_size_mb=round(checkpoint_size_mb, 2), load_time_ms=round(load_time * 1000, 2), checkpoint_metrics=checkpoint.get('metrics', {}))
        except KeyError as e:
            logger.error('Checkpoint is missing required keys', checkpoint_path=path, missing_key=str(e), available_keys=list(checkpoint.keys()))
            raise
        except Exception as e:
