    ...
    def _init_neuro_symbolic(self: "GraphBuilderHost", config: Any) -> None:
        ...
        try:
            ...
            self.symbolic_agent = SymbolicReasoningAgent(
                config=ns_config, neural_fallback=self._neural_fallback_for_symbolic, logger=self.logger
            )
            self.symbolic_extension = SymbolicAgentGraphExtension(
                reasoning_agent=self.symbolic_agent, config=SymbolicAgentNodeConfig(), logger=self.logger
            )
            ...
            self.neuro_symbolic_mcts = NeuroSymbolicMCTSIntegration(
                config=mcts_ns_config, reasoning_agent=self.symbolic_agent, logger=self.logger
            )
