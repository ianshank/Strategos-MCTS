...
class CoreNodesMixin:
    ...
    def _node_retry(
        self, node_name: str, fn: Callable[[], Any], on_retry: Callable[[Exception, int], None] | None = None
    ) -> Callable[[], Any]:
        ...
        async def handler(state: AgentState) -> dict:
            ...
            try:
                ...
                return {
                    ...
                    "agent_outputs": [{"agent": f"adk_{name}", "response": content, "confidence": confidence}],
                }
            except Exception as e:
                self.logger.error(f"ADK agent {name} failed: {e}")
                return {
                    "agent_outputs": [
                        {"agent": f"adk_{name}", "response": f"Error executing {name}: {e}", "confidence": 0.0}
                    ]
                }
