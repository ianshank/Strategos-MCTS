...
- **2.4 Uncertainty-Aware Subgoal Selection (S3) 🟢 SPECS 1–3 LANDED:** A four-spec roadmap ports an MDN dispersion signal into graph MCTS candidate selection. Delivered: the pluggable candidate-scoring seam (`src/framework/mcts/scoring.py`, `GRAPH_MCTS_CANDIDATE_SCORER`, byte-for-byte identity default), the torch-guarded coarse-dynamics MDN + numpy dispersion reference (`src/models/coarse_dynamics.py`), and the risk-averse scorer `score = value − λ·dispersion` (`src/framework/mcts/risk_scoring.py`, `ENABLE_UNCERTAINTY_SUBGOAL_PENALTY`, off by default). **Remaining (Spec 4, `strategos_subgoal_uncertainty_benchmark`, draft):** the ON-vs-OFF A/B benchmark + related-work citation, and the deferred wiring that attaches *real* coarse-state dispersion onto graph candidate metadata (today's placeholder candidates make the penalty a value-ranking no-op). Blocked on agreeing the A/B regression threshold and verifying the arXiv source id.
