...
> Structural/architectural detail. For *why* this system exists, its scope, non-goals, and
> invariants, see **[`../CHARTER.md`](../CHARTER.md)** — the two documents answer different
> questions and neither restates the other.
...
| **LLM adapters** | Provider-agnostic clients (OpenAI, Anthropic, LM Studio) over a shared resilience layer (`src/adapters/llm/resilience.py` — `CircuitBreaker`) with tenacity retries. LM Studio base URLs are normalized (`localhost` → `127.0.0.1`, `/v1` suffix); live adapter QA is opt-in (`REQUIRE_LMSTUDIO=1`), never `tests/unit/`. |
...
**Gate status:** this table previously hardcoded a point-in-time snapshot (2026-07-20: 305 source
files, 93.82% coverage) that had drifted from reality — see `docs/reviews/2026-07-31-charter-alignment-audit.md`
for the pattern. Gate status is not duplicated here; see **[`docs/STATUS.md`](STATUS.md)**, the
project's single source of truth for measured test/coverage/lint status, regenerated by the
`coverage-baseline` skill rather than hand-edited.

### Evidence chain (`src/tools/`)

A fourth quality axis, orthogonal to lint/type/test: **does the documentation's description of the
system match the system?** The tools are deliberately small, pure-stdlib, and injected with a
`repo_root` and a clock so they are testable and usable from a scratch copy.

```mermaid
flowchart LR
    subgraph Sources["Committed sources of truth"]
        R["README.md<br/>CHARTER.md §2"]
    ...
    subgraph Tools["src/tools/ (console scripts)"]
        CL["claim_ledger<br/>schema + promotion rule"]
    ...
    subgraph Gates["Enforcement"]
        CI["ci.yml :: spec-validate"]
        MK["make gate"]
        HK[".claude/hooks/evidence_gate.py<br/>(advisory, PostToolUse)"]
    end

    ART["artifacts/status.json<br/>(build output, git-ignored)"]
...
The load-bearing properties, each enforced by a test that reconstructs the shape it forbids:

| Property | Where | Consequence of violating it |
| --- | --- | --- |
| A `PROVEN` row needs a resolvable evidence path and a verify command | `src/tools/claim_ledger.py` | `spec-validate` fails; CI also proves the gate rejects a falsified row |
| A reported number carries a provenance label from a closed vocabulary | `src/tools/status_artifact.py` (`ResultEntry.__post_init__`) | the entry cannot be constructed, so it cannot reach the artifact |
...
audits search semantics, both read-only, and neither may re-run a measurement to make it pass. See
`docs/plans/EVIDENCE_FIRST_PROGRAM.md` for the milestone sequence (E0–E5) this implements.
